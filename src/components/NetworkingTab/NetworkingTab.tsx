...
export const NetworkingTab = () => {
  ...
  const completedTopics = currentUser?.completedTopics?.['networking'] || {};

  useEffect(() => {
    setLoading(true);
    const result = loadData('networking');
    if (result) {
      setData(result as NetworkingData);
    }
    setLoading(false);
  }, []);
  ...
  const handleReset = () => {
    if (currentUser && window.confirm('Are you sure you want to reset all progress for Networking?')) {
      dispatch(resetUserCategoryProgress('networking'));
    }
  };
